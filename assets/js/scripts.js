...
// ADD NEW CURRENCY FROM GENERAL SETTINGS PAGE
	...
	//ENABLE / DISABLE USER
	$('.userTable').on('click', '.user_action', function() {
		
		var row = $(this).closest("tr");
		var indice = $(this).closest("tr").index();
		var user = "#users_userId"+(indice+1);
		var userId = $(user).val();
		job=confirm(ALERT_REMOVE_ROW);
			if(job!=true){
				//cancel the action of deactivating a user
				return false;
			} else {
				//deactivate user
				$.ajax({
					...
					type: 'POST',
					dataType: 'json',
					url: base_url+'user/activationAction',
					success: function(result) {
						window.location.href = base_url+'users/list';
					},
					error: function(xhr, status, error){
						var errorMessage = xhr.status + ': ' + xhr.statusText
						alert('Error - ' + errorMessage);

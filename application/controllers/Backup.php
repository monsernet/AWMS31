...
class Backup extends CI_Controller {
	...
	*/
	function index() {
		...
		$data['title'] = $this->lang->line('backups');
		$data['db_list'] = $this->global_model->RetreiveData('backup',['backup_type' => 1]);
		$data['file_list'] = $this->global_model->RetreiveData('backup',['backup_type' => 2]);
		$data['applicationRow'] = $this->global_model->RetreiveRow('application_settings', ['id' => 1]);
		
		$this->load->view('inc/header',$data);
		$this->load->view('inc/topheader',$data);
		$this->load->view('inc/sidebar');
		$this->load->view('backups/backup');
		...
		if ($file_format == 1) {
			...
			if (strlen($backup)) {
				if (!write_file($file, $backup)) {
					$this->handle_error($this->lang->line('error_create_backup') . $file_name);
				} else {
					$this->handle_success($this->lang->line('file_name') . $file_name . '.<br/> ');
					$this->handle_success($this->lang->line('db_backup_success').'<br/>');
					...
					if ($saved_data !== NULL) {
						$this->handle_success($this->lang->line('db_backup_saved_success'));
						//$this->handle_success($this->lang->line('db_download_url') . anchor($this->db_download_url . $saved_data, $this->lang->line('download'), array('class' => 'download')));
						//$this->handle_success($this->lang->line('db_delete_url') . anchor($this->db_delete_url . $saved_data, $this->lang->line('delete'), array('class' => 'delete', 'onclick' => "return confirm('Are you sure want to delete this file ?')")));
					} else {
						if (file_exists($file)) {
							unlink($file);
						}
						$this->handle_error($this->lang->line('error_saving_db_backup') . $file_name);
					}
				}
			} else {
				$this->handle_error($this->lang->line('error_getting_backup_db') . $file_name);
			}
			...
			if (strlen($backup)) {
				if (!write_file($file, $backup)) {
					...
					$this->handle_success($this->lang->line('file_name') . $file_name . '.<br/> ');
					$this->handle_success($this->lang->line('db_backup_failure').'<br/>');
					$date_arr = explode('_', $key_name3);
					$date = $date_arr[0] . '-' . $date_arr[1] . '-' . $date_arr[2] . ' ' . $date_arr[3] . ':' . $date_arr[4] . ':' . $date_arr[5];
					$saved_data = $this->backup->save_backup_details($file_name, $file_path, $backup_type);
	...
	*/
	private function get_site_backup($file_path, $backup_type, $file_format = 1) {
		...
		if ($file_format == 1) {
			$file_name = md5($key_name) . '_files.zip';
			...
			if ($zipped == 1) {
				$this->handle_success($this->lang->line('file_name') . $file_name . '.<br/> ');
				$this->handle_success($this->lang->line('files_backup_success').'<br/>');
				...
				if ($saved_data !== NULL) {
					$this->handle_success($this->lang->line('files_backup_saved_success'));
					//$this->handle_success($this->lang->line('files_download_url') . anchor($this->site_download_url . $saved_data, $this->lang->line('download'), array('class' => 'download')));
					//$this->handle_success($this->lang->line('files_delete_url') . anchor($this->site_delete_url . $saved_data, $this->lang->line('delete'), array('class' => 'delete', 'onclick' => "return confirm('Are you sure want to delete this file ?')")));
				} else {
					if (file_exists($file)) {
						unlink($file);
					}
					$this->handle_success($this->lang->line('error_saving_files_backup') . $file_name);
				}
			} else {
				$this->handle_error($this->lang->line('error_create_backup') . $file_name);
			}
		} else if ($file_format == 2) {
			$file_name = md5($key_name) . '_files.tar.gz';
			$zipped = $this->zip->archive($file_path . $file_name);
			$this->zip->clear_data();

...
class User extends CI_Controller {
	...
	public function activationAction() {
		$userId = $this->global_model->sanitizeString($this->input->post('userId'));
		$currentStatus = $this->global_model->getItem('users', 'id', $userId, 'status');
		if($currentStatus==0){
			//if current status=0 ( banned ) => activate the user
			//by changing status to 1
			$status = 1;
			//$newaction reserved for the displaying the alert
			$newaction = $this->lang->line('activated');
		} else {
			//if current status=1 ( active ) => bann the user
			//by changing status to 0
			$status = 0;
			//$newaction reserved for the displaying the alert
			$newaction = $this->lang->line('banned');
		}
		$action = array (
			'status' => $status
		);
		$this->db->where(['id' => $userId])->update('users', $action);	
		
		$this->session->set_flashdata('userbanned', '<div class="alert alert-success"><i class="fa fa-check-circle"></i> '.$this->lang->line('user_successfully').$newaction.'</div>'); 
		$result = 'action';
		echo json_encode($result);
	}
	...
	{
		$data['activedrop'] = 'users';
		$data['activemenu'] = 'banned';
		$data['title'] = $this->lang->line('banned_users');
		$data['applicationRow'] = $this->global_model->RetreiveRow('application_settings', ['id' => 1]);
		$data['users'] = $this->global_model->RetreiveData('users',['status' => 0]);
		$this->load->view('inc/header');
		$this->load->view('inc/topheader', $data);
		$this->load->view('inc/sidebar');
		$this->load->view('users/banned',$data);
		$this->load->view('inc/footer');
	}
